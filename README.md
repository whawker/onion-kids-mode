...
- **Parent menu** (hold **SELECT+START 3 s** → PIN): exit Kids Mode, add
  ...
  **A**/**START** applies, and the header previews the remaining time before
  ...
  brightness**, flip **auto-resume**, **switch to another child**, **add a
  child**, or **change the PIN** — all without leaving the launcher.
...
- **The first time you add a second child**, Kids Mode asks for that
  ...
  now their profile didn't need one. Their existing progress is kept as-is
